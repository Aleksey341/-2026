(() => {
  ...
  H.registerUpdate(dt=>{
    ...
    const swing=moving?Math.sin(walking)*.46:0;limbs.armL.rotation.x=BABYLON.Scalar.Lerp(limbs.armL.rotation.x,swing,.18);limbs.armR.rotation.x=BABYLON.Scalar.Lerp(limbs.armR.rotation.x,-swing,.18);limbs.legL.rotation.x=BABYLON.Scalar.Lerp(limbs.legL.rotation.x,-swing*.72,.18);limbs.legR.rotation.x=BABYLON.Scalar.Lerp(limbs.legR.rotation.x,swing*.72,.18);torso.position.y=1.18+(moving?Math.abs(Math.sin(walking*2))*.016:0);
