(() => {
  ...
  // v4.1.4: lower emissive values keep people/architecture from blowing out into white blobs.
  const M={
    floor:mat('hr4-floor','#10171e',.005,.62,.25),
    wall:mat('hr4-wall','#1a242c',.004,.35,.44),
    dark:mat('hr4-dark','#060a0f',.002,.5,.3),
    glass:mat('hr4-glass','#9fd8ee',.015,.08,.14,.16),
    white:mat('hr4-white','#d9e6eb',.055,.08,.24),
    cyan:mat('hr4-cyan','#69cee8',.42,.08,.2),
    teal:mat('hr4-teal','#66cdb3',.36,.08,.21),
    violet:mat('hr4-violet','#a99ae8',.39,.08,.2),
    gold:mat('hr4-gold','#d9bc77',.34,.18,.26),
    red:mat('hr4-red','#e27686',.37,.08,.22),
    amber:mat('hr4-amber','#d6a64f',.24,.18,.3)
  };
  ...
  for(let z=4;z<=76;z+=4){const strip=box('hr4-floor-line-'+z,{width:.035,height:.018,depth:2.6},V(0,.015,z),M.white,null,true);strip.visibility=.16;}
  for(const side of [-1,1]){
    box('hr4-atrium-wall-'+side,{width:.3,height:7.5,depth:12.5},V(side*8.8,3.75,1.8),M.wall);
    for(let z=8;z<=73;z+=8){box(`hr4-gallery-pier-${side}-${z}`,{width:.28,height:5.4,depth:.42},V(side*4.9,2.7,z),M.wall);box(`hr4-gallery-light-${side}-${z}`,{width:.035,height:3.6,depth:.08},V(side*4.73,2.7,z-.18),M.white,null,true).visibility=.20;}
  }
  const core=cyl('hr4-atrium-core',{diameter:2.7,height:7.4,tessellation:36},V(0,3.7,1.5),M.glass);core.visibility=.28;
  cyl('hr4-atrium-core-inner',{diameter:.62,height:7.7,tessellation:28},V(0,3.85,1.5),M.cyan,null,true).visibility=.36;
  const ring=cyl('hr4-atrium-ring',{diameter:6.8,height:.12,tessellation:48},V(0,6.8,1.5),M.white,null,true);ring.scaling.z=.74;ring.visibility=.22;
  ...
  for(const section of data.sections){
    const {x,z}=section.room; const root=new BABYLON.TransformNode('hr4-room-'+section.id,scene);root.position.set(x,0,z);roomRoots[section.id]=root;
    const accent=accentFor(section.id);
    if(section.final){
      box('hr4-future-floor',{width:15,height:.15,depth:11},V(0,-.07,0),M.white,root).visibility=.65;
    ...
    for(const s of [-1,1]){box(`hr4-room-side-${section.id}-${s}`,{width:.22,height:5.8,depth:9.0},V(s*5.15,2.9,0),M.glass,root).visibility=.30;box(`hr4-room-edge-${section.id}-${s}`,{width:.035,height:4.1,depth:.08},V(s*5.02,2.85,-4.0),accent,root,true).visibility=.74;}
    box('hr4-room-ceiling-'+section.id,{width:9.8,height:.12,depth:8.4},V(0,5.7,0),M.dark,root);
    box('hr4-room-light-'+section.id,{width:5.8,height:.035,depth:.18},V(0,5.58,.5),accent,root,true).visibility=.38;
    ...
    if(section.id==='recruitment'){
      ...
      cyl('hr4-ai-beam',{height:1.5,diameterTop:.12,diameterBottom:.9,tessellation:24},V(0,1.35,.6),M.glass,root,true).visibility=.22;
      ...
      cyl('hr4-award-stage',{diameter:4.8,height:.32,tessellation:48},V(0,.16,.8),M.wall,root);
