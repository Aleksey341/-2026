...
capture=null;
// Stylised adult heroine: navy suit jacket, white blouse, straight trousers, smooth low bun.
capture=null;
const player=new T.Group();player.position.set(.9,0,1.63);scene.add(player);const body=new T.Group();player.add(body);
profile([[.145,0],[.19,.07],[.225,.22],[.23,.36],[.195,.43],[.13,.47]],M.purple,0,.88,0,body,1,.64);profile([[.105,0],[.11,.035],[.097,.125],[.092,.13]],M.blouse,0,1.32,0,body,1,.8);profile([[.152,0],[.16,.04],[.155,.085]],M.purple,0,.87,0,body,1,.75);box(.006,.19,.009,M.metal,0,1.33,-.109,body);box(.025,.035,.013,M.metal,0,1.24,-.12,body);box(.09,.012,.004,M.blouse,0,1.28,-.118,body);
ell(.19,.14,.13,M.pants,0,.87,0,body);
const legs=[],arms=[];for(const side of[-1,1]){const leg=new T.Group();leg.position.set(side*.096,.85,0);body.add(leg);profile([[.085,0],[.09,.12],[.083,.32],[.088,.57],[.091,.74]],M.pants,0,-.74,0,leg,1,.94);rounded(.157,.072,.275,.052,M.shoe,0,-.817,-.049,leg);rounded(.162,.028,.283,.05,M.black,0,-.835,-.049,leg);for(let j=0;j<3;j++)box(.09,.006,.012,M.metal,0,-.735,-.065-j*.025,leg);legs.push(leg);
const arm=new T.Group();arm.position.set(side*.206,1.275,0);arm.rotation.z=side*.12;body.add(arm);profile([[.053,0],[.073,.055],[.079,.17],[.076,.28],[.061,.38]],M.purple,0,-.37,0,arm,1,.95);cyl(.052,.05,.065,M.blouse,0,-.389,0,arm);ell(.042,.073,.029,M.skin,0,-.474,-.002,arm);ell(.021,.041,.024,M.skin,-side*.031,-.451,-.022,arm);arms.push(arm)}
